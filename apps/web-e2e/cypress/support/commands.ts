...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      registerViaUi(args: { email: string; password: string; name: string }): Chainable<void>;
      interceptRegister(): Chainable<void>;
      testLoginAfterConfirmation(email: string, password: string): Chainable<void>;
...
Cypress.Commands.add(
  ...
  ({ email, password, name }: { email: string; password: string; name: string }) => {
    ...
    cy.get('[data-cy="register-password"]').clear().type(password, { log: false });
    cy.get('[data-cy="register-confirm-password"]').clear().type(password, { log: false });
...
Cypress.Commands.add('testLoginAfterConfirmation', (email: string, password: string) => {
  cy.interceptLogin();
...
Cypress.Commands.add('confirmEmailViaSupabaseAPI', (confirmationLink: string) => {
